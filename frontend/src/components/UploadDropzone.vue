...
<template>
	<label class="flex cursor-pointer flex-col gap-5 rounded-[28px] border border-dashed border-[#c7d2e2] bg-gradient-to-br from-[#f7faff] to-white p-6 transition hover:border-[#1a73e8] hover:bg-[#f4f8ff]" @drop="onDrop" @dragover="onDragOver">
		...
		<div class="space-y-3">
			<p class="text-xs font-semibold uppercase tracking-[0.14em] text-[#1a73e8]">Upload pipeline</p>
			<h2 class="text-2xl font-semibold text-[#202124]">Drag, drop, and stream directly to the target cloud</h2>
			<p class="max-w-3xl text-sm leading-6 text-[#5f6368]">
				File payloads pass through the Node.js proxy, while metadata is mirrored locally and
				progress is reported by WebSocket.
			</p>
		</div>
